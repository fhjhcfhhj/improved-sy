...
local function ResolveImageAsset(input)
    ...
    if type(input) == "string" then
        ...
        elseif input:match("^http") then
            ...
            if assetFunc and writefile then
                local ok, result = pcall(function()
                    local req = (syn and syn.request) or (http and http.request) or http_request or request
                    local body = req({Url = input, Method = "GET"}).Body
                    local fname = "VIPBG_" .. tostring(math.random(10000, 99999)) .. ".png"
                    writefile(fname, body)
                    return assetFunc(fname)
                end)
...
local function CreateVIPBorder(target, thickness, customColors)
    ...
    stroke.Name = "VIPBorderStroke"
    stroke.Thickness = thickness or VIPBorderConfig.Thickness
...
RunService.Heartbeat:Connect(function()
    for _, data in ipairs(ActiveGradients) do
        if data.MainGradient and data.MainGradient.Parent then
            data.MainGradient.Rotation = (data.MainGradient.Rotation + (data.Speed or VIPBorderConfig.Speed)) % 360
        end
        if data.GlowGradient and data.GlowGradient.Parent then
            data.GlowGradient.Rotation = (data.GlowGradient.Rotation + (data.Speed or VIPBorderConfig.Speed) * 0.65) % 360
        end
    end
end)
...
function Fenglib:CreateWindow(Config)
    ...
    local BackgroundUrl = Config.Background
    ...
    if BackgroundUrl and BackgroundUrl ~= "" then
        local resolved = ResolveImageAsset(BackgroundUrl)
        if resolved and resolved ~= "" then
            local bgImage = Instance.new("ImageLabel")
            bgImage.Name = "WindowBackgroundImage"
    ...
    OpenButton.Image = "rbxassetid://121122779270617"
    ...
    local function createSection(parent, text, icons, defaultOpen)
        ...
        child.Button = function(_, btnText, callback)
            ...
            CreateVIPBorder(Btn, VIPBorderConfig.Thickness)
        ...
        child.Toggle = function(_, toggleText, default, callback)
            ...
            CreateVIPBorder(Tile, VIPBorderConfig.Thickness)
        ...
        child.Textbox = function(_, boxText, placeholder, callback)
            ...
            CreateVIPBorder(Frame, VIPBorderConfig.Thickness)
        ...
        child.Input = function(_, inputText, default, callback, options)
            ...
            CreateVIPBorder(InputFrame, VIPBorderConfig.Thickness)
        ...
        child.Label = function(_, labelText)
            ...
            CreateVIPBorder(LabelFrame, VIPBorderConfig.Thickness)
        ...
        child.SubLabel = function(_, subLabelText)
            ...
            CreateVIPBorder(SubLabelFrame, VIPBorderConfig.Thickness)
        ...
        child.Paragraph = function(_, headerText, bodyText)
            ...
            CreateVIPBorder(ParaFrame, VIPBorderConfig.Thickness)
        ...
        child.Image = function(_, config)
            ...
            CreateVIPBorder(imageFrame, VIPBorderConfig.Thickness)
